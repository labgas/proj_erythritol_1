...
where time > -10;
var tri;
histogram tri / normal (mu=est sigma=est) lognormal (sigma=est theta=est zeta=est);
run;
/*no transformation*/
...
where time > -10; 
model BoxCox(tri/parameter=0) = identity(z);
run;
...
where time > -10;
var bc_tri;
histogram bc_tri / normal (mu=est sigma=est);
run;
/*  */
...
model bc_tri = condition | time / ddfm=kr2 influence solution residual;
