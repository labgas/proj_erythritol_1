...
where time > -10;
var UA;
histogram UA / normal (mu=est sigma=est) lognormal (sigma=est theta=est zeta=est);
run;
/*not a disaster*/
...
where time > -10; 
model BoxCox(UA/parameter=0) = identity(z);
run;
...
where time > -10;
var bc_UA;
histogram bc_UA/ normal (mu=est sigma=est);
run;
/*looks ok */
...
model bc_UA = condition | time / ddfm=kr2 influence solution residual;
